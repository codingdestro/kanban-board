...
interface ElectronAPI {
  ...
  writeState: (data: any) => Promise<boolean>;
}
...
export const useKanbanStore = create<KanbanStore>((set, get) => {
  const saveState = async (updatedBoards: { [id: string]: Board }) => {
  ...
  return {
    ...
    hydrateStore: async () => {
      ...
      try {
        const cached = await window.electronAPI.readState();
        ...
        console.error('Failed to load store from local JSON file', e);
      }
      set({ isHydrated: true });
    },
